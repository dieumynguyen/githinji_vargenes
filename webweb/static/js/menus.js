...
function writeMenuRight() {
    ...
    var menuR7 = menuR_subdiv.append("div").attr("id","menuR7").attr("class", "menu_row_right");
    ...
    menuR7.text("Highlight all nodes");
    menuR7.append("input")
        .attr("id", "highlight_all_nodes")
        .attr("class", "floatRight")
        .attr("type", "checkbox")
        .on("change", toggleHighlightAll);
}

function toggleHighlightAll() {
    highlight_all_nodes = !highlight_all_nodes;

    toggleDynamics(d3.select("#onoffSelect"));

    d3.select("#onoffSelect").property("checked", function() {
        return !highlight_all_nodes;
    });
    
    force.nodes().forEach(function(d, i) {
        (highlight_all_nodes) ? highlightText(d, 4) : unHighlightText();
    });
}
